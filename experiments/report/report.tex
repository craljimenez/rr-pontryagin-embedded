...
All images are resized to $512 \times 512$ pixels using bilinear
...
the source repository.  During training, random horizontal and vertical flips
(each with probability 0.5) are applied.
...
All three segmentation heads share a \textbf{deep fully-convolutional network
...
\[
  \mathcal{L} = \mathcal{L}_{\text{CE}}
              + \lambda_W\,   \mathcal{L}_{\text{cone-W}}
...
\paragraph{Cone penalty on weights $\mathcal{L}_{\text{cone-W}}$.}
Weight vectors that lie on the \emph{null cone} $\{u : \Jprod{u}{u} = 0\}$
produce degenerate classifiers (the J-hyperplane has no definite normal).
The penalty keeps them away from the cone:
\[
...
\[
  ...
  = \mathrm{ReLU}\!\left(\|\mathbf{z}_-\|_2 - \|\mathbf{z}_+\|_2\right),
\]
where $\mathbf{z}_+$ and $\mathbf{z}_-$ are the positive and negative
subspace components.  This prevents the negative subspace from dominating
the representation.
...
\paragraph{Scheduler.}  Cosine annealing from $\eta$ to $\eta_{\min} = 10^{-5}$
over $E_{\max} = 60$ epochs.
...
epochs.  The checkpoint with the highest validation macro mIoU is kept.

\paragraph{Batch size.}  2 images per GPU step (limited by the $512^2$
resolution and GPU memory).

\paragraph{Hardware.}  Single CUDA GPU.
...
starts).  Each trial trains for 12 epochs with early stopping at patience 6.
The search spaces are model-specific.

\subsection{Common to all models}
\begin{center}
\begin{tabular}{lll}
\toprule
Parameter & Range & Prior \\
\midrule
\texttt{lr}           & $[10^{-5},\; 10^{-2}]$ & log-uniform \\
\texttt{weight\_decay}& $[10^{-6},\; 10^{-3}]$ & log-uniform \\
\texttt{backbone\_depth} & $\{3, 4, 5\}$        & uniform \\
\bottomrule
\end{tabular}
\end{center}

\subsection{Hyperbolic-specific}
\begin{center}
\begin{tabular}{lll}
\toprule
Parameter & Range & Prior \\
\midrule
\texttt{hyperbolic\_c} & $[0.1,\; 2.0]$ & log-uniform \\
\bottomrule
\end{tabular}
\end{center}

\subsection{Pontryagin-specific}

\begin{center}
\begin{tabular}{llll}
\toprule
Parameter & Range & Prior & Interpretation \\
\midrule
$\kap$ (\texttt{kappa})
  & $\{1,\ldots,64\}$ & log-uniform
  & Pontryagin signature $=n_{\text{srf}}$ \\
$\dpoly$ (\texttt{d\_poly})
  & $\{1,\ldots,10\}$ & uniform
  & SRF polynomial degree \\
\texttt{rff\_multiplier}
  & $\{1,2,3,4\}$ & uniform
  & $n_{\text{rff}} = m \cdot C_{\text{out}}$ \\
$\lambda_W$
  & $[10^{-6},\; 0.5]$ & log-uniform
  & cone penalty weight \\
$\lambda_{\text{topo}}$
  & $[10^{-6},\; 0.2]$ & log-uniform
  & topographic penalty weight \\
$\lambda_{\text{bal}}$
  & $[10^{-6},\; 0.1]$ & log-uniform
...
$\{1,\ldots,64\}$ ensures that small values ($\kap \in \{1,\ldots,8\}$, where
the indefinite component is modest) receive the same sampling probability as
large values ($\kap \geq 32$, where the negative subspace is dominant).  A
uniform prior over this range would bias exploration towards large $\kap$,
which is undesirable given that the interaction between the subspace size and
the polynomial degree is not yet characterised.
...
design choices simultaneously.  The current implementation separates them:
$\kap = n_{\text{srf}}$ is the signature, and $\dpoly$ is the degree of
$K_-(\mathbf{x},\mathbf{y}) = (\mathbf{x}^\top\mathbf{y})^{\dpoly}$, searched
independently.

\paragraph{Gradient-tuned parameters.}
The RFF bandwidth $\sigma$ is \emph{not} part of the HPO space.  It is
reparametrised as $\sigma = \exp(\log\sigma)$ and updated by gradient descent,
with the stored unit-variance frequencies $\omega_i \sim \mathcal{N}(0,I)$
scaled by $1/\sigma$ at each forward pass.
